...
export async function connectRedis(): Promise<Redis> {
  ...
  const instance = new Redis(Config.redisUrl, {
  ...
  instance.on('connect', () => log.info('Redis подключён ✓'));
  instance.on('error', (err) => log.error('Redis ошибка', err));
  instance.on('close', () => log.warn('Redis соединение закрыто'));
