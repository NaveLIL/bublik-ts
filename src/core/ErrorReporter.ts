...
class ErrorReporterManager {
  ...
  init(client: Client, errorChannelId?: string | null): void {
    ...
    this.flushTimer = setInterval(() => {
      void this.flush().catch((err) => log.error('Ошибка flush ErrorReporter', err));
    }, FLUSH_INTERVAL_MS);
  ...
  async shutdown(): Promise<void> {
    ...
    // Дожидаемся текущей отправки и выгружаем все батчи.
    await this.flush(true);
    if (this.queue.length > 0) {
      log.warn(`ErrorReporter завершён с ${this.queue.length} неотправленными отчётами`);
    }
  }
  ...
  report(report: ErrorReport): void {
    ...
    const errorObj = normalizeError(report.error);
    ...
    if (this.queue.length < MAX_QUEUE_SIZE) {
      this.queue.push({ ...report, error: errorObj });
    }

    // Critical — отправляем немедленно
    if (report.severity === ErrorSeverity.Critical) {
      void this.flush().catch((err) => log.error('Немедленный flush ErrorReporter не удался', err));
    }
  }
  ...
  private async flush(drain = false): Promise<void> {
    do {
      if (this.queue.length === 0 && !this.flushPromise) return;

      let active = this.flushPromise;
      if (!active) {
        active = this.flushBatch();
        this.flushPromise = active;
      }

      const progressed = await active;
      if (this.flushPromise === active) this.flushPromise = null;
      if (!drain || !progressed) return;
    } while (this.queue.length > 0);
  }

  /** Send one queue-head batch. Reports are removed only after Discord accepts it. */
  private async flushBatch(): Promise<boolean> {
    ...
    try {
      ...
      if (!channel) {
        // No configured destination is intentional. A configured but temporarily
        // unavailable destination keeps its queue for a later retry.
        if (!this.errorChannelId) this.queue.length = 0;
        return false;
      }
      ...
      this.stats.failed += Math.min(this.queue.length, 5);
      log.error('Не удалось отправить ошибки в Discord-канал', err);
      return false;
    }
  }
  ...
  private async resolveChannel(): Promise<TextChannel | null> {
    ...
    if (this.channelResolved && Date.now() - this.lastResolveAttemptAt < 30_000) return null;
    if (!this.errorChannelId || !this.client) {
      this.channelResolved = true;
      return null;
    }

    this.lastResolveAttemptAt = Date.now();
  ...
  private buildEmbed(report: ErrorReport): EmbedBuilder {
    const error = normalizeError(report.error, 'Unknown');
