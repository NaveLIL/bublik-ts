...
export class BublikClient extends Client {
  ...
  async shutdown(): Promise<void> {
    ...
    // ErrorReporter — отправляем остаток очереди пока ещё есть соединение
    try { await errorReporter.shutdown(); } catch (e) { logger.warn('ErrorReporter shutdown failed', e as Error); }
